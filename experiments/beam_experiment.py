...
def timer(duration, run_time):
    ...
    while time.time() - start < duration:
        print(f"run: {n_runs}")
        output = subprocess.check_output(["timeout", f"{run_time}", "python3", "main.py", "Rushhour6x6_1.csv", "beam", "--silent"])
    ...
    with open('results/beam/beam_experiment.csv', "w", newline='') as file:
